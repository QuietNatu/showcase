...
Right now, I think that the best approach to take is to mock the network layer and to provide an in-memory database with mock data generated by a random data generator. A good way to achieve this would be to use **Mock Service Worker (MSW)** to mock the requests, **@msw/data** to provide the in-memory database, and **Faker.js** to generate the data used in that in-memory database. By using a database instead of directly mocking, the api requests, we can abstract that way requests are handled from the domain models used by the app. This way, api endpoints can be refactored more easily without breaking existing tests.
...
Proper API documentation is essential both for consumers of the API and the developers of it. APIs should be defined in documents using standardized formats like OpenAPI Specification. This makes it possible for tools to interpret these documents and provide utilities like client and server code generation (like orval or @openapitools/openapi-generator-cli) or create documentation webpages (like swagger).
These tools should always be used as they allow for clearer API contracts to be defined and to avoid mismatches between client and server code. These avoid common issues that arise from manually writing the api endpoints:

- mismatched endpoint names or paths
- mismatched models
- improper handling of endpoint versions or deprecations
