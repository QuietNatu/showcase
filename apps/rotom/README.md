...
### Official Angular test libraries do not have template code coverage

https://github.com/angular/angular-cli/issues/25563

Although Angular component templates are compiled into JavaScript, Angular does not support measuring their code coverage. Angular seems to be the only UI framework that currently has this limitation.

I think this reflects poorly on the quality of a framework when the testability of its tools and APIs is an afterthought. This lack of coverage has created an avalanche of issues and poor testing practices that still affect many, if not most, Angular projects.

What is most worrying is that it is possible to measure code coverage of component templates in Angular. The Analog Vitest plugin allows this by enabling AOT compilation.

The most common issue I see is that teams do not know how to write maintainable UI tests. They do not even try to render the component templates and instead rely on implementation details, interacting with internal component variables and functions. This will obviously lead to maintainability issues, as a simple refactor with no functional changes can break the tests.

Angular has since noticed this issue and now recommends in their style guide using marking internal variables/functions as **protected**. However, a simple style guide change is not enough, and stricter enforcement should be implemented.
